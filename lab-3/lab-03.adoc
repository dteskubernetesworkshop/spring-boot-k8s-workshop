...
4. Open a browser to `http://localhost:NODE_PORT/`, replacing `NODE_PORT` with the `NodePort` you found in step 3.  When we ran step 3, we got port `30012` so I'll browse to `http://localhost:30012`.
...
3. Kubernetes forwards 30012 to `hellonode-service`'s port 8080.

4. The service looks for pods matching `app: pages`, and randomly picks a pod.

5. The service forwards the traffic to the chosen pod's port 8080.

6. The pod forwards traffic to the container's port 8080.

7. Node is listening in the container on port 8080.  It got the message, and returns the html.
